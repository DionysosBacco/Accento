...
{
    //cout<<"OFDevice, Arc, left:"<<left<<", top:"<<top<<", right:"<<right<<", y:"<<bottom<<endl;
    ...
    ofColor color = Color2ofColor(fFillColor);
	ofPath path;
...
{
    //cout<<"OFDevice, Triangle, x1:"<<x1<<", y1:"<<y1<<", x2:"<<x2<<", y2:"<<y2<<endl;
	ofFill();
...
{
	if (count < 2) return;
    //cout<<"OFDevice, Polygon, x:"<<xCoords[0]<<", yCoords:"<<yCoords[0]<<", count:"<<count<<endl;
...
{ 
	//cout << "openFrameworksDevice::SetOrigin:" << x << " y:" << y << endl;
...
{ 
	//cout << "openFrameworksDevice::OffsetOrigin" << x << " y:" << y <<  endl;
	ofTranslate(x, y);
	fXOrigin += x; fYOrigin += y; 
}
...
void openFrameworksDevice::NotifySize( int width, int height ) { 
	...
	//cout << "openFrameworksDevice::NotifySize: allocating FBO: width" << fWidth << "height" << fHeight << endl;
	drawCache.allocate(fWidth, fHeight, GL_RGBA, 8);
	drawCache.begin();
	ofClear(255,255,255, 0);
	drawCache.end();
